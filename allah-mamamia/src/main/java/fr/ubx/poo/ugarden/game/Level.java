        ...
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++) {
                ...
                switch (mapEntity) {
                    ...
                    case Princess: {
                        Decor grass = new Grass(position);
                        grass.setBonus(new Princess(position, grass));
                        decors.put(position, grass);
                        break;
                    }
                    case Key: {
                    ...
                    case DoorPrevOpened:{
                        decors.put(position,new Door_opened(position));
                        break;
                    }
                    case Bee:{
                        Decor grass = new Grass(position);
                        decors.put(position, grass);
                        Collection<Position> UpdatedBeePosition = entities.updateBeePositionCollection();
                        game.setBeePosition(UpdatedBeePosition);
                        break;
                    }
                    default:
